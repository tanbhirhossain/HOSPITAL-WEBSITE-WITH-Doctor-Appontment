...
// Inertia-te current route/path check kora hoy usePage() diye
const currentUrl = computed(() => page.url)

const doctorsCurrent = computed(() => currentUrl.value.startsWith('/find-doctor'))
const departmentsCurrent = computed(() => currentUrl.value.startsWith('/departments'))
const aboutCurrent = computed(() => currentUrl.value.startsWith('/about'))
const blogsCurrent = computed(() => currentUrl.value.startsWith('/blogs'))
...
function submitSearch() {
  ...
  // vue-router er router.push() er poriborte Inertia router.get()
  router.get('/find-doctor', { q: term })
}
...
<template>
  ...
  <header class="relative z-40 bg-white site-header">
    ...
    >
      <!-- RouterLink er poriborte Inertia Link -->
      <Link class="shrink-0" href="/" aria-label="AMZ Hospital home" @click="closeMenu">
        <img
      ...
      <div class="hidden items-center gap-9 xl:flex" aria-label="Main menu">
        <Link class="nav-link" href="/find-doctor" :data-current="doctorsCurrent">Doctors</Link>
        <Link class="nav-link" href="/departments" :data-current="departmentsCurrent">Departments</Link>
        <Link class="nav-link" href="/about" :data-current="aboutCurrent">About</Link>
        <Link class="nav-link" href="/blogs" :data-current="blogsCurrent">Blogs</Link>
      </div>
      ...
      <div class="ml-auto hidden items-center gap-7 lg:flex">
        ...
        <div class="inline-flex h-[42px] items-center rounded-full bg-[#fafafa] p-1 text-[13px] shadow-[0_2px_8px_rgba(0,0,0,0.08)]" aria-label="Language">
          <button
            ...
            :class="language === '?????' ? 'bg-white font-semibold text-[#020605] shadow-sm' : 'text-[#868686]'"
            type="button"
            @click="language = '?????'"
          >?????</button>
          <button
    ...
    <div v-show="menuOpen" id="mobile-menu" class="border-t border-[#eeeeee] bg-white px-5 pb-5 pt-3 lg:hidden">
      <div class="mx-auto flex max-w-[520px] flex-col gap-1">
        <Link class="mobile-nav-link" href="/find-doctor" @click="closeMenu">Doctors</Link>
        <Link class="mobile-nav-link" href="/departments" @click="closeMenu">Departments</Link>
        <Link class="mobile-nav-link" href="/about" @click="closeMenu">About</Link>
        <Link class="mobile-nav-link" href="/blogs" @click="closeMenu">Blogs</Link>
